...
- `GET /ready` returns JSON `{"status":"ready","checks":{...}}` (200) or `not_ready` (503). It pings the database and Redis (when `REDIS_URL` is set) and does not check the schema, so `/health` is the deploy gate.
