...
function defaultMigrationDatabaseUrl(layers: StarterLayers, projectName: string): string | null {
...
function postgresAppRoleEnvNotes(layers: StarterLayers): string[] {
  ...
  return [
    ...
    layers.tenancy === "rls"
      ...
      : "# This app is not TENANCY_DRIVER=rls; still do not point runtime traffic at the postgres superuser.",
  ];
}
...
function renderEnvExample(projectName: string, layers: StarterLayers): string {
  const lines = [
    ...
    ...(layers.database === "postgres"
      ? [`MIGRATION_DATABASE_URL=${defaultMigrationDatabaseUrl(layers, projectName)}`]
      : []),
    ...(layers.database === "sqlite"
      ? []
      : [
          "# Optional. Point runtime at a different database than DATABASE_URL.",
          "# APP_DATABASE_URL=",
        ]),
...
function renderPackageJson(
  ...
  const coreDeps: Record<string, string> = options.workspaceDependencies
    ...
    : {
        "@getstrata/bootstrap": "^1.1.4",
        "@getstrata/cli": "^1.1.4",
        "@getstrata/core": "^1.1.4",
      };
  coreDeps.eta = "^4.6.0";
...
The app uses the database named in \`DATABASE_URL\` and creates it on first migrate when the connection user may. Set \`APP_DATABASE_URL\` only when migrations and the app should target a different database than \`DATABASE_URL\`.${
        usesComposePostgres(layers)
          ? ` \`.env.example\` points \`DATABASE_URL\` at \`${GENERATED_POSTGRES_APP_ROLE}\` (\`NOSUPERUSER\` \`NOBYPASSRLS\`), including \`--no-docker\`. \`db/ensure-postgres-app-role.sql\` is repeatable on an existing volume; Compose init still runs on first empty volume. The \`postgres\` superuser is for CREATE ROLE / GRANT / migrate and Adminer.`
          : ""
      }
`
}
