...
function localEnvVars(layers: StarterLayers, services: DockerServiceName[]): string[] {
  const vars: string[] = [];
  for (const name of services) {
    if (name === "postgres") {
      vars.push("DATABASE_URL");
    } else if (name === "mysql") {
      vars.push(layers.database === "mysql" ? "DATABASE_URL" : "MYSQL_URL");
    } else if (name === "redis") {
      vars.push("REDIS_URL");
...
function renderSupportingToolsReadme(layers: StarterLayers): string {
  ...
  if (localOn.length > 0) {
    ...
    const envVars = localEnvVars(layers, localOn)
      .map((name) => `\`${name}\``)
      .join(", ");
    lines.push(
