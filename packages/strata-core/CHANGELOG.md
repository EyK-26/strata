...
- API CSRF middleware runs on guest and session mutating requests. `POST /api/v1/auth/login` and `POST /api/auth/token` require double-submit CSRF. Session-mutating API (logout after cookie login) requires CSRF. `GET /api/v1/auth/csrf` Set-Cookies the HttpOnly CSRF cookie and returns the token in JSON. Failed Bearer still does not skip CSRF or fall through to the session. SCIM and SAML ACS skip CSRF by path because those authenticators run after CSRF. CORS allowlists `X-CSRF-Token`.
