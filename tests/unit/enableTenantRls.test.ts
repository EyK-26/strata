...
describe("enableTenantRls", () => {
  ...
  test("emits FORCE RLS and a tenant isolation policy", () => {
    ...
    expect(sql).not.toContain("app_bypass_identifier()");
  });
  ...
  test("emits FORCE RLS for user-owned tables without tenant_id", () => {
    ...
    expect(sql).toContain("sessions.id::text = app_bypass_identifier()");
    expect(sql).toContain("sessions.user_id::text = app_bypass_identifier()");
    ...
    const memberships = enableUserOwnedRlsSql("memberships");
    expect(memberships).toContain("memberships.user_id::text = app_bypass_identifier()");
  });
  ...
  test("generated bootstrap includes user-owned session and token tables", () => {
    ...
    expect(sql).toContain("api_tokens.token_hash = app_bypass_identifier()");
    expect(sql).toContain("auth_one_time_tokens.token_hash = app_bypass_identifier()");
    expect(sql).not.toMatch(
      /CREATE POLICY tenant_isolation ON notes[\s\S]*app_bypass_identifier\(\)[\s\S]*CREATE POLICY tenant_isolation ON users/,
    );
  });
});
