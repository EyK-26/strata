...
describe("starter template CLI", () => {
  test("depends on @getstrata/cli and uses strata scripts", async () => {
    ...
    expect(packageJson.dependencies["@getstrata/cli"]).toBe("^1.0.0");
    expect(packageJson.dependencies["@getstrata/core"]).toBe("^1.0.0");
    expect(packageJson.dependencies["@getstrata/bootstrap"]).toBe("^1.0.0");
