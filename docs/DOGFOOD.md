...
Wave 5 made HiroApp the developer default (`bun run dev`). Waves 6–9 ported staff identity, teams, RLS, SCIM, webhooks, audit, and billing onto HiroApp. Wave 10 points CI at HiroApp (OpenAPI, smoke, `bun run test:hiroapp:coverage`) and deletes the WorkHub application tree. Waves 11–14 add the staff hiring pipeline, department CRUD, candidate apply/restore, and first-class interviews. Wave 15 adds a position service so staff can open, update, stop, and reopen hiring without deleting the seat. Wave 16 assigns staff interviewers onto a position.
