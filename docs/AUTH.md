...
| `FEATURE_OAUTH` | Optional `--oauth-github` generates GitHub cookie login (`GET /auth/github`, callback, login link, JIT + MFA gate). OIDC uses `createAuthorization()` plus a PKCE handshake; there is no generated OIDC cookie login. Inbound OIDC ID tokens are RS256 via JWKS. App JWTs are HS256 (`signJwt`). Multi-valued `aud` requires `azp` equal to the client id. `at_hash` is verified when present; omitted `access_token` plus `at_hash` throws. GitHub OAuth always reads `/user/emails` and throws unless a verified address exists (unverified `profile.email` is ignored). |
