...
| Server HTMX | `server-htmx` | HTML views under `/organizations`, `/projects`, `/tasks`, `/search`, `/reports`, `/account` (profile name/email, password + MFA + recovery codes + logout other devices), `/confirm-password` (export/delete), `/two-factor-challenge` (Fortify 2FA), `/notifications`, `/billing`, `/webhooks`, `/forgot-password`, etc. |
