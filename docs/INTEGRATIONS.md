...
Set the matching `GITHUB_*` or `OIDC_*` variables. OIDC `getAuthorizationUrl()` throws. Use `createAuthorization()` (async) and pass the handshake to `exchangeCode()`. Authorization, token, and JWKS URLs come from discovery. Inbound OIDC ID tokens are verified as RS256 via discovery JWKS (`iss` / `aud` / `azp` / `at_hash` / `exp` / `nbf` / `nonce`). App JWTs stay HS256. Multi-valued `aud` requires `azp` equal to the client id. `at_hash` is verified when present; omitted `access_token` plus `at_hash` throws. Missing or unverified email throws. GitHub OAuth uses `safeFetch`, always reads `/user/emails`, and throws unless a verified address exists (no `{login}@users.noreply.github.com`, and an unverified `profile.email` is ignored).
