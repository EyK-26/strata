...
WorkHub stays the in-monorepo reference app. It pins `APP_KEY_PREFIX=workhub` and `APP_NAME=WorkHub`. Published defaults are `strata` / `Strata` (`strata_session`, `strata_csrf`) when those env vars are unset. HMAC `SessionGuard` is for API/token apps; sibling HTMX apps should use `CookieSessionStore`.
...
HMAC `SessionGuard` (`@getstrata/core/auth/sessionGuard`) is for API/token apps. It reads `${APP_KEY_PREFIX}_session` (default `strata_session`; override with `SESSION_COOKIE_NAME`) and loads the user through an `AuthUserDirectory`. `appCookieName()` / `appDevSecret()` on `@getstrata/core/runtime/appKeyPrefix` derive cookie names and local secret fallbacks from the same prefix. Core exported names (`SESSION_COOKIE`, `CSRF_COOKIE`, `FLASH_COOKIE`, `INTENDED_URL_COOKIE`, `PASSWORD_CONFIRM_COOKIE`) are `appCookieName(...)` at import time and do not track later env changes. WorkHub’s `MFA_CHALLENGE_COOKIE` lives in the app module, not core.
...
WorkHub ships `sessions` (`0031_create_sessions`: `id` text PK, `user_id` → `users` cascade, `expires_at`) so `CookieSessionStore.create` / `read` / `destroy` can run against the same Postgres as HMAC login. WorkHub HTMX login still uses HMAC cookies (`APP_KEY_PREFIX=workhub` → `workhub_session`) — do not swap it onto `CookieSessionStore` without rewriting every web session test. HMAC login dual-writes a `sessions` row. `SessionGuard` rejects a valid HMAC cookie when `hasActiveBrowserSession` returns false (row deleted). Cookies issued before that dual-write have no row and must re-login. HTML and JSON `logout-other-devices` / password change delete `sessions` rows (JSON also sets `session_valid_after`). JSON `GET`/`DELETE /users/me/sessions` list and revoke rows.
...
|--------|---------|----------|
| HMAC session (`SessionGuard`) | `${APP_KEY_PREFIX}_session` (`strata_session`; WorkHub pins `workhub_session`) | `SESSION_COOKIE_NAME` |
...
| Redis cache / queue / throttle keys | `strata:` | `APP_KEY_PREFIX` |
| SMTP EHLO host | `strata.local` | `MAIL_EHLO` |
| SIEM `event_type` / CEF vendor | `strata.audit` / `Strata` | `SIEM_EVENT_TYPE` / `APP_NAME` |
| GitHub OAuth user-agent | `strata` | `APP_USER_AGENT` |
| OTEL `service.name` | `strata-api` | `OTEL_SERVICE_NAME` |
| Webhook HMAC header | `x-strata-signature` | `WEBHOOK_SIGNATURE_HEADER` |
| OpenAPI title / SDK class | `Strata API` / `StrataClient` | `APP_NAME` / `APP_SDK_CLASS` |
...
`createRoutes` / `createWebRoutes` / `schedule` assemble the in-repo dogfood app (SCIM, WorkHub feature flags). Sibling HTMX apps should use `buildWebModuleRoutes` (optionally `seedRoutes: createHealthRoutes(deps, { pingOnHealth: true })`). `wrapWebGuest` defaults to `/`; WorkHub passes `resolveHomePath()`.
...
WorkHub still calls the published helper plus its own extras from `App.serve()` / `queue:work`. `createAppContext()` does not. A production HTMX app with `DATABASE_URL`, `SESSION_SECRET` (32+ chars), `AUTH_DEV_HEADERS=false`, and feature flags off can call `assertProductionSecrets()` without API tokens.
