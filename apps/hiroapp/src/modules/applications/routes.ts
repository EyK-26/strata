  ...
  return {
    ...
    "/api/applications/:id": {
      GET: wrapApi(dependencies, async (request) => {
        ...
        const payload = await loadApplicationDetail(id);
        const response = jsonResponse(payload);
        if (isEtagEnabled()) {
          return applyConditionalGet(request, response, etagFromResource(application.toObject()));
        }
        return response;
      }),
    },
    ...
    "/api/applications/:id/end": {
      POST: wrapApi(dependencies, async (request) => {
        ...
        if (!isCandidate(actor.role_id)) {
          ...
          const relatedPosition = await Application.newFromRecord(application).position();
    ...
    "/api/applications/:id/move": {
      POST: wrapApi(dependencies, async (request) => {
        ...
        await runInTransaction(async () => {
          ...
          const position = await Application.newFromRecord(application).position().first();
          if (!position) {
            throw new UnprocessableEntityError("Application has no position.");
          }
          ...
          await position.update({ user_id: application.user_id, hiring: false });

          const siblings = await Position.newFromRecord(position.toObject()).applications();
          const rejectedIds: number[] = [];
          for (const sibling of siblings) {
            const siblingId = Number(sibling.id);
            if (
              siblingId !== Number(application.id) &&
              Number(sibling.get("status_id")) !== STATUS.ENDED
            ) {
    ...
    "/api/export/applications": {
      GET: wrapApi(dependencies, async (request) => {
        ...
        return jsonResponse({ count: rows.length, data: rows });
      }),
    },
  };
}
