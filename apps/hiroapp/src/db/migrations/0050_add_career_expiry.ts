...
const allowed = "status IN ('published', 'unpublished', 'expired')";
const previous = "status IN ('published', 'unpublished')";

const migration: Migration = {
  name: "0050_add_career_expiry",
  async up(db) {
  ...
  async down(db) {
    await db.unsafe(`UPDATE career_postings SET status = 'unpublished' WHERE status = 'expired'`);
    await db.unsafe(`ALTER TABLE career_postings DROP CONSTRAINT IF EXISTS ${constraint}`);
