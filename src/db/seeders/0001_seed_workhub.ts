...
const seeder: Seeder = {
  ...
  async run(db) {
    ...
    await db`
      ...
      VALUES
        (1, 1, 'Platform Rewrite', 'active'),
        (2, 1, 'Legacy Migration', 'draft'),
        (3, 2, 'Docking Simulator', 'active')
      ON CONFLICT (id) DO NOTHING
    `;
    await db`
      SELECT setval(
    ...
    await db`
      ...
      VALUES
        (1, 1, 'Design module registry', 'done', 3),
        (2, 1, 'Implement query layer', 'in_progress', 4),
        (3, 2, 'Map legacy endpoints', 'todo', 2),
        (4, 3, 'Simulate docking sequence', 'in_progress', 5)
      ON CONFLICT (id) DO NOTHING
    `;
    await db`
      SELECT setval(
    ...
    await db`
      INSERT INTO comment (id, task_id, body)
      VALUES
        (1, 1, 'Registry supports provider boot phases.'),
        (2, 1, 'Auto-registration added to make:module.'),
        (3, 2, 'BaseRepository covers CRUD and eager loads.'),
        (4, 4, 'Needs collision detection at 30m.')
      ON CONFLICT (id) DO NOTHING
    `;
    await db`
      SELECT setval(
